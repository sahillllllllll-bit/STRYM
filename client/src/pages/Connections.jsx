...
const Connections = () => {
  ...
  <p className='text-slate-600'>Manage Your Network And Make New Friendss</p>
</div>
...
{
  dataArray.map((tab)=>(
    <button onClick={()=> setCurrentTab(tab.label)} key={tab} className={`flex items-center px-3 py-3 text-sm rounded-md transitions-colors ${currentTab== tab.label ? 'bg-white font-medium text-black ':'text-gray-500  hover:text-black '}`}>
...
{tab.count!== undefined && (
  ...
  {/* connection-shown-layout  */}


<div className='flex flex-wrap gap-6 mt-6'>
  {dataArray.find((item)=> item.label === currentTab).value.map((user)=>(
 <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
   <img src={user.profile_picture} alt=""  className=' rounded-full w-12 h-12 shadow mx-auto' />
