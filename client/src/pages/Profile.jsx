...
user.cover_photo && <img src={user.cover_photo} alt='' className='w-full h-full object-cover' />
     ...
     {/* user-info  */}
  <UserProfileInfo user={user} posts={posts} profileId={profileId} setShowEdit={setShowEdits}/>
     </div>
  ...
  <div className="bg-white rounded-xl shadow p-1 flex max-w-md mx-auto">
    {
      ["posts","media","likes"].map((tab)=>(
          <button key={tab} onClick={()=>setActiveTabs(tab)} className={` flex-1 px-4 py-2 text-sm font-medium rounded-lg transition-colors cursor-pointer ${activeTabs===tab ? "bg-indigo-600 text-white" : "text-gray-600 hover:text-gray-900"} `}>
